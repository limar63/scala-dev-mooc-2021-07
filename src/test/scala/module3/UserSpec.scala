...
object UserSpec extends DefaultRunnableSpec{
  override def spec = suite("User spec")(
    testM("notify user"){
      ...
      val sendMailMock = EmailServiceMock.SendMail(equalTo(Email(EmailAddress("test@test.com"), Html("Hello here"))), unit)
      ...
      } yield assert(value)(anything)).provideSomeLayer[TestConsole with Console](layer)
    }
  )
}
